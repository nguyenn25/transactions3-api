...
### 6. Challenges
*Docker Networking:* I got super stuck on a Docker networking error for a while. I kept getting `MONGODB_URI is missing` and then a `getaddrinfo ENOTFOUND db` error when I tried to run my seed script. I eventually figured out it was because the database name in my `docker-compose.yml` didn't perfectly match the connection string in my `.env` file. Also, I realized I had to run the seed command explicitly inside the local Docker container shell instead of my regular Mac terminal so it could actually reach the database.
